...
function onNpcScriptFileList(e,data){var found,files,root,query,out,i,s;if(!canUseEditor(e.player)){pushBrowser(e.player,"npcScriptFileList",{ok:false,root:"",files:[],error:"No permission"});return;}found=listEcmaScriptFiles();files=found.files||[];root=found.root||"";query=String(data.query||"").toLowerCase();out=[];if(query){for(i=0;i<files.length;i++){s=files[i];if(String(s).toLowerCase().indexOf(query)>=0)out.push(s);}}else out=files;pushBrowser(e.player,"npcScriptFileList",{ok:!found.error,root:root,files:out,error:found.error||""});}
...
function listEcmaScriptFiles(){var roots=findEcmaScriptRoots(),out=[],seen={},i,j,root,files,path,primaryRoot="";if(!roots.length)return {root:"",files:[],error:"No ecmascript folder found. Create customnpcs/scripts/ecmascript in the world save or server root."};for(i=0;i<roots.length;i++){root=roots[i];if(!primaryRoot)primaryRoot=String(root.getAbsolutePath()).replace(/\\/g,'/');files=[];walkJsFiles(root,root,files);for(j=0;j<files.length;j++){path=String(files[j]||"");if(!path||seen[path])continue;seen[path]=true;out.push(path);}}out.sort();return {root:primaryRoot,files:out};}
